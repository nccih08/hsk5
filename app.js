...
function changeVocabPage(dir) {
  vocabCurrentPage += dir;
  renderVocabTable();
...
function renderVocabTable() {
  ...
  const totalPages = Math.ceil(words.length / VOCAB_PER_PAGE) || 1;
  ...
  const displayed = words.slice(startIndex, startIndex + VOCAB_PER_PAGE);
  ...
  const countLabel = words.length > 0 ? `Menampilkan ${startIndex + 1} - ${startIndex + displayed.length} dari ${words.length} kata` : 'Tidak ada kata ditemukan';
  document.getElementById('vocab-count').textContent = countLabel;
  
  const pageInfo = document.getElementById('vocab-page-info');
  if (pageInfo) pageInfo.textContent = `${vocabCurrentPage} / ${totalPages}`;
}
